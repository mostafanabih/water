                                ...
                                <div class="ibox-title">
                                    <table class="table table-bordered">
                                        ...
                                        <tr>
                                            ...
                                            <td> {{$company->name}}</td>
                                            <td><img src="{{$company->getImgDisplay()}}" alt="company" style="width: 200px;"></td>
                                            <td>
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-warning update" data-company_id="{{$company->id}}"  data-name="{{$company->name}}" data-mobile="{{$company->mobile}}" data-commercial_register_image="{{$company->commercial_register_image}}" data-image="{{$company->image}}" data-email="{{($company->responsible)?$company->responsible->email : "--"}}" data-mobile2="{{($company->responsible) ? $company->responsible->mobile:"--"}}" data-user_name="{{($company->responsible)?$company->responsible->user_name:"--"}}" data-name2="{{($company->responsible)?$company->responsible->name:"--"}}"  >تعديل</button>
                                                <button type="button" class="btn btn-success change" data-my_id="{{$company->id}}">تغيير كلمه المرور </button>
                                                <a class="btn btn-info" href="{{url('admin/order?company_id='.$company->id)}}">الطلبات </a>
                                                <button type="button" class="btn btn-primary" data-toggle="modal" ><a href="{{url('admin/product/'.$company->id)}}" style="color: inherit">المنتجات  </a></button>
                                                <button type="button" class="btn " ><a href="{{url('admin/representative/'.$company->id)}}" style="color: inherit"> المندوبين</a></button>
                                                <button type="button" class="btn btn-danger stop" data-my_id="{{$company->id}}" data-status="@if($company->active == 'not_active'){{'active'}}@else{{'not_active'}}@endif">@if($company->active == 'not_active'){{'تفعيل'}}@else{{'ايقاف'}}@endif</button>
                                                <button type="button" class="btn btn-danger delete" data-my_id="{{$company->id}}">حذف</button>
                                            </td>
                                        </tr>
                                        @endforeach
                                        </tbody>
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="ibox float-e-margins">
                                            <div class="col-sm-12">
                                                <h3>اضافه</h3>
                                                <form action="{{route('company.store')}}" method="post" class="form-horizontal" enctype="multipart/form-data">
                                                    {{csrf_field()}}
                                                    <h4 class="">بيانات الشركه</h4>
                                                    <div class="form-group"><label class="col-sm-4 control-label">  اسم الشركه </label>
                                                    <div class="col-sm-8" style="margin-bottom: 1em"><input type="text" name="name" value="{{old('name')}}" class="form-control"></div>
                                                </div>
                                                <div class="form-group"><label class="col-sm-4 control-label">  رقم الجوال  </label>
                                                    <div class="col-sm-8" style="margin-bottom: 1em"><input type="text" name="mobile" value="{{old('mobile')}}" class="form-control"></div>
                                                </div>
                                                ...
                                                <div class="form-group"><label class="col-sm-6 control-label">  صوره الشركه  </label>
                                                    ...
                                                    <div class="form-group"><label class="col-sm-4 control-label">  اسم المسؤل </label>
                                                        <div class="col-sm-8" style="margin-bottom: 1em"><input type="text" name="name2" value="{{old('name2')}}" class="form-control"></div>
                                                    </div>
                                                    <div class="form-group"><label class="col-sm-4 control-label">  اسم المستخدم </label>
                                                        <div class="col-sm-8" style="margin-bottom: 1em"><input type="text" name="user_name" value="{{old('user_name')}}" class="form-control"></div>
                                                    </div>
                                                    ...
                                                    <div class="form-group"><label class="col-sm-4 control-label">  البريد الالكتروني  </label>
                                                        <div class="col-sm-8" style="margin-bottom: 1em"><input type="text" name="email" value="{{old('email')}}" class="form-control"></div>
                                                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(".update").click(function () {
                ...
                $('#editform').attr('action', '{{url("/admin/company")}}'+'/'+company_id);
            ...
            $(".delete").click(function () {
                ...
                /* set action attribute to new url */
                $('#deleteform').attr('action', '{{url("/admin/company")}}' + '/' + id);
                /* open the modal */
            ...
            $(".change").click(function () {
                ...
                var id = $(this).data("my_id");
                /* set action attribute to new url */
                $('#changeform').attr('action', '{{url("/admin/company-change-password")}}' + '/' + id);
                /* open the modal */
            ...
            $(".stop").click(function(){
                ...
                /* set action attribute to new url */
                $('#stopform').attr('action', '{{url("admin/stopcompany")}}' + '/' + id );
