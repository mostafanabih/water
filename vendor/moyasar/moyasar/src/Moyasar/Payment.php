...
class Payment
{
    ...
    public static function create($amount, $source, $description = "", $currency = "SAR", $shop_id=null, $order_id=null)
    {

        if(is_null($shop_id)){
            $route = route('confirm_moyasar_payment', ['shop_id' => session('shop_id')]);
        }else{
            $route = route('api.confirm_moyasar_payment', ['shop_id' => $shop_id, 'order_id' => $order_id]);
        }
