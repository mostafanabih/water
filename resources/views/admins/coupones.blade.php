    ...
    <div class="wrapper wrapper-content">
                        ...
                        <div class="row">
                            ...
                            <form class="form-inline" action="">
                                <div class="col-lg-2">
                                    ...
                                    <input value="{{request('search')}}" type="text" placeholder="ابحث عن كوبون  " class="form-control" name="search">
                            ...
                            <div class="ibox float-e-margins">
                                <div class="ibox-title">
                                    <table class="table table-bordered">
                                        ...
                                        <tr>
                                            <td>{{$coupone->code}}</td>
                                            <td>{{($coupone->company)? $coupone->company->name:"--"}} </td>
                                            <td> {{$coupone->percentage}}</td>
                                            <td> {{$coupone->date_from}}</td>
                                            <td> {{$coupone->date_to}}</td>
                                            <td>
                                                <button class="btn btn-info edit"
                                                        data-my_id="{{$coupone->id}}"
                                                        data-code="{{$coupone->code}}"
                                                        data-percentage="{{$coupone->percentage}}"
                                                        data-date_from="{{$coupone->date_from}}"
                                                        data-date_to="{{$coupone->date_to}}"
                                                        data-company_id="{{$coupone->company_id}}">تعديل </button>
                                                <button type="button" class="btn btn-danger delete" data-my_id="{{$coupone->id}}">حذف</button>
                                            </td>
                                        </tr>
                                            @endforeach
                                        </tbody>
                            ...
                            <div class="ibox float-e-margins">
                                <div class="col-sm-12">
                                    ...
                                    <form action="{{route('coupon.store')}}" method="post" class="form-horizontal">
                                        ...
                                        {{--<div><label> <input type="radio" checked="" value="option1" id="optionsRadios1" name="optionsRadios"> مفعل  </label></div>--}}
                                        {{--<div><label> <input type="radio" value="option2" id="optionsRadios2" name="optionsRadios"> غير مفعل  </label></div>--}}
        ...
        <div class="ibox float-e-margins">
            <div class="ibox-title text-center ">
                {{$coupones->onEachSide(1)->appends(Request::capture()->except('page'))->render() }}
            </div>
        </div>
    </div>
@endsection
